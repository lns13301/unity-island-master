...
public class ReinforceUI : MonoBehaviour
{
    ...
    public void doItOn()
    {
        ...
        doItMessage.text = "강화를 진행하시겠습니까?" + NEW_LINE
            ...
            + "보유금액 : " + dialogManager.playerData.money + " Money" + NEW_LINE
    ...
    public void moveSlotToHammer()
    {
        ...
        if (dialogManager.playerData.money < (item.reinforce + 1) * 2)
        {
            popUp.SetActive(true);
        ...
        //누르는 순간 재화소모
        dialogManager.playerData.money -= (item.reinforce + 1) * 2;
        dialogManager.savePlayerDataToJson();
    ...
    public Item progressReinforce(Item instanceItem)
    {
        ...
        if (instanceItem.satietyPoint != 0)
        {
            float value = Mathf.Ceil((float)instanceItem.satietyPoint * reinforceAmount[instanceItem.reinforce]);
            if (value < 0)
            {
                value = 0;
            }

            instanceItem.satietyPointReinforce += (int)(value + instanceItem.reinforce * 3);
        }
        if (instanceItem.moisturePoint != 0)
        {
            float value = Mathf.Ceil((float)instanceItem.moisturePoint * reinforceAmount[instanceItem.reinforce]);
            if (value < 0)
            {
                value = 0;
            }

            instanceItem.moisturePointReinforce += (int)(value + instanceItem.reinforce * 3);
        }
        if (instanceItem.catharsisPoint != 0)
        {
            float value = Mathf.Ceil((float)instanceItem.catharsisPoint * reinforceAmount[instanceItem.reinforce]);
            if (value < 0)
            {
                value = 0;
            }

            instanceItem.catharsisPointReinforce += (int)(value + instanceItem.reinforce * 3);
        }
        if (instanceItem.fatiguePoint != 0)
        {
            float value = Mathf.Ceil((float)instanceItem.fatiguePoint * reinforceAmount[instanceItem.reinforce]);
            if (value < 0)
            {
                value = 0;
            }

            instanceItem.fatiguePointReinforce += (int)(value + instanceItem.reinforce * 3);
        }

        if (instanceItem.toolEff != 0)
        {
            float value = instanceItem.toolEff * reinforceAmount[instanceItem.reinforce];
            if (value < 0)
            {
                value = 0;
            }

            instanceItem.toolEffReinforce += (float)(value + instanceItem.reinforce * 0.2);
        }
        if (instanceItem.skillEff != 0)
        {
            float value = instanceItem.skillEff * reinforceAmount[instanceItem.reinforce];
            if (value < 0)
            {
                value = 0;
            }

            instanceItem.skillEffReinforce += (float)(value + instanceItem.reinforce * 0.2);
        }
