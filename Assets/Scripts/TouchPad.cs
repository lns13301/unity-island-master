...
public class TouchPad : MonoBehaviour
{
    ...
    public void showPanelNumber()
    {
        try
        {
            ...
            ShopInformation.instance.itemPrice.text = getNumber() * ShopInformation.instance.item.price + " Isle";
            ShopInformation.instance.itemSellPrice.text = getNumber() * ShopInformation.instance.item.price + " Isle";

            // 상자 패널
            if (GameObject.Find("Canvas").GetComponent<ChestUI>().chestSet.activeSelf && ChestInformation.instance.item.count < Calculator.fomattingToInteger(touchPanelNumberString))
            {
                touchPanelNumberString = "" + ChestInformation.instance.item.count;
            ...
            inputField[1].text = Calculator.numberToFormatting(Calculator.fomattingToInteger(touchPanelNumberString));

            ChestInformation.instance.itemPrice.text = getNumber() * ChestInformation.instance.item.price + " Isle";
            ChestInformation.instance.itemSellPrice.text = getNumber() * ChestInformation.instance.item.price + " Isle";
        }
