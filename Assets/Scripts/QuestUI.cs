...
public class QuestUI : MonoBehaviour
{
    ...
    public void setStartableQuest()
    {
        ...
        for (int i = 0; i < dialogManager.playerData.startQuest.Count; i++)
        {
            GameObject go = Instantiate(questTitlePanel);
            go.transform.SetParent(content.transform);
            go.GetComponent<QuestTitleButton>().questId = dialogManager.playerData.startQuest[i];

            // 타이틀 제목
            GameObject title = go.transform.GetChild(0).gameObject;
            title.GetComponent<Text>().text = QuestDatabase.instance.questDB[dialogManager.playerData.startQuest[i]].questTitle;
        }
    }
    ...
    public void setProceedingQuest()
    {
        ...
        for (int i = 0; i < dialogManager.playerData.currentQuest.Count; i++)
        {
            GameObject go = Instantiate(questTitlePanel);
            go.transform.SetParent(content.transform);
            go.GetComponent<QuestTitleButton>().questId = dialogManager.playerData.currentQuest[i].questId;
            //go.GetComponent<Text>().text = QuestDatabase.instance.questDB[dialogManager.playerData.currentQuest[i].questId].questTitle;

            // 타이틀 제목
            GameObject title = go.transform.GetChild(0).gameObject;
            title.GetComponent<Text>().text = QuestDatabase.instance.questDB[dialogManager.playerData.currentQuest[i].questId].questTitle;
        }
    }
    ...
    public void setCompleteQuest()
    {
        ...
        for (int i = 0; i < dialogManager.playerData.clearQuest.Count; i++)
        {
            GameObject go = Instantiate(questTitlePanel);
            go.transform.SetParent(content.transform);
            go.GetComponent<QuestTitleButton>().questId = dialogManager.playerData.clearQuest[i];
            //go.GetComponent<Text>().text = QuestDatabase.instance.questDB[dialogManager.playerData.clearQuest[i]].questTitle;

            // 타이틀 제목
            GameObject title = go.transform.GetChild(0).gameObject;
            title.GetComponent<Text>().text = QuestDatabase.instance.questDB[dialogManager.playerData.clearQuest[i]].questTitle;
        }
    }
    ...
    public void showQuestInformationPanel(int questId)
    {
        ...
        if (buttonState == QuestState.Proceeding)
        {
            ...
            for (int i = 0; i < dialogManager.playerData.currentQuest.Count; i++)
            {
                if (dialogManager.playerData.currentQuest[i].questId == questId)
                {
                    currentQuest = dialogManager.playerData.currentQuest[i];
                }
            }
